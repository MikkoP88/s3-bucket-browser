...
- **Explorer layout**: toolbar, back/forward/up history, breadcrumb, folder tree sidebar, sortable details grid with per-column visibility (main grid and side panel, configured in Settings), status bar.
...
- **Versions**: every row carries a live version-count badge (⟲ n) — click it for the object's timeline or the folder-level **Directory Versions** overview; restore-as-latest, one-click **undo delete** for delete markers (also from the ⛔ marker window), permanent destroy and bulk purge of noncurrent versions — in the GUI dialog and via Shift+Del.
...
- **Upload menu**: one **Upload** command everywhere — **Files… (Ctrl+U)** for the native multi-select file dialog, **Folder…** for a whole directory tree (walked and uploaded recursively).
