...
- **Versioned-delete choice**: on versioned buckets Delete asks first — add a **delete marker** (default, everything restorable) or **delete permanently** (purges every version and marker under the selection, typed confirmation gates it). The grid badges marker state per row: ⛔ n on files with markers in their history, "all deleted" on folders with nothing live beneath.
