# Verification report — s3b v1.1.0-beta.16

**RELEASE GATE: PASS — 83 PASS · 2 SKIP · 0 FAIL** — full matrix in 726s.

**Version stamp:** `1.1.0-beta.16` — binary-verified by the run itself:
...
**Verified commit:** `50004ee` — Verification: 9 more extreme-level rows for critical data sources (76 -> 85)
**OS:** Windows_NT 10.0.26100 (x64)
**Node:** v24.12.0
**Run:** id `vermua11f10`, bucket `verify-mua11f10`, started 2026-09-20T16:36:54.093Z
**Command:** `node scripts/verify.mjs --release v1.1.0-beta.16`
...
| CLI-S3-33 | CLI | SSE-S3 server-side encryption (--sse AES256) | S3 (MinIO) | cp --sse AES256 uploads with the SSE header; the object round-trips byte-identical (SKIP records a provider gap when the engine rejects SSE) | **SKIP** — provider gap — SSE rejected: error: C:\Projects\s3-bucket-browser\testartifacts\verification\fixtures\data\root-1.txt: operation error S3: PutObject, https response error StatusCode: 501, RequestID: 18D7144384BFE3C2, HostID: dd9025bab4ad464b049177c95eb6ebf374d3b3fd1af9251148b658df7ac2e3e8, api error NotImplemented: Server side encryption specified but KMS is not configured (KMS not configured for a server side encrypted objects) |
...
| CLI-X-02 | CLI | Multi-file copy local→FTP | FTP | cp -r fixture tree (10 files: unicode, empty file, nested dirs) → xf://vermua11f10/tree | PASS |
...
- **CLI-S3-33 SKIP** — provider gap — SSE rejected: error: C:\Projects\s3-bucket-browser\testartifacts\verification\fixtures\data\root-1.txt: operation error S3: PutObject, https response error StatusCode: 501, RequestID: 18D7144384BFE3C2, HostID: dd9025bab4ad464b049177c95eb6ebf374d3b3fd1af9251148b658df7ac2e3e8, api error NotImplemented: Server side encryption specified but KMS is not configured (KMS not configured for a server side encrypted objects)
- CLI-S3-01 PASS — added, tested, listed, mirrored
- CLI-S3-02 PASS — verify-mua11f10 created, versioning on
...
- CLI-S3-29 PASS — 32 MiB multipart round-trip sha256-identical in 2.9s
...
- CLI-RES-01 PASS — correct listing in 2.2s under latency
- CLI-RES-02 PASS — clean classified failure
- CLI-RES-03 PASS — failed cleanly in 8.6s
...
- GUI-01 PASS — v3 stack up, 1.1.0-beta.16
...
```bash
node scripts/verify.mjs --release v1.1.0-beta.16
```
